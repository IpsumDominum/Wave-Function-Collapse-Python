...
def get_hash_to_code(pattern_set,ground,hash_frequency_dict,GROUND=False):
    ...
    ground_coded = np.zeros(ground.shape,dtype=int)
    if(GROUND):
        for i in range(len(ground)):
            ground_coded[i] = hash_to_code_dict[ground[i]]
    return pattern_code_set,hash_to_code_dict,avg_color_set,ground_coded,code_frequencies
