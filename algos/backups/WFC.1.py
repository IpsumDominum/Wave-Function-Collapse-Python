...
def wfc_run(
    ...
    for j in range(len(cropped_list)):
        ...
        # up
        up_idx = ((row_dx - N) % cropped_sets.shape[0]) * cropped_sets.shape[
            1
        ] + column_dx
        j_neighbours[0] = cropped_list[up_idx]

        # down
        down_idx = ((row_dx + N) % cropped_sets.shape[0]) * cropped_sets.shape[
            1
        ] + column_dx
        j_neighbours[1] = cropped_list[down_idx]

        # left
        left_idx = row_dx * cropped_sets.shape[1] + (
            (column_dx - N) % cropped_sets.shape[1]
        )
        j_neighbours[2] = cropped_list[left_idx]

        # right
        right_idx = row_dx * cropped_sets.shape[1] + (
            (column_dx + N) % cropped_sets.shape[1]
        )
        j_neighbours[3] = cropped_list[right_idx]

        # upleft
        upleft_idx = ((row_dx - N) % cropped_sets.shape[0]) * cropped_sets.shape[1] + (
            (column_dx - N) % cropped_sets.shape[1]
        )
        j_neighbours[4] = cropped_list[upleft_idx]

        # upright
        upright_idx = ((row_dx - N) % cropped_sets.shape[0]) * cropped_sets.shape[1] + (
            (column_dx + N) % cropped_sets.shape[1]
        )
        j_neighbours[5] = cropped_list[upright_idx]

        # downleft
        downleft_idx = ((row_dx + N) % cropped_sets.shape[0]) * cropped_sets.shape[
            1
        ] + ((column_dx - N) % cropped_sets.shape[1])
        j_neighbours[6] = cropped_list[downleft_idx]

        # downright
        downright_idx = ((row_dx + N) % cropped_sets.shape[0]) * cropped_sets.shape[
            1
        ] + ((column_dx + N) % cropped_sets.shape[1])
