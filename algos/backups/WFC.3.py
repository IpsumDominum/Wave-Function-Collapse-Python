...
def wfc_run(
    ...
    print("ENCODING...")
    # ENCODE EVERYTHING
    for i in range(math.ceil((input_padded.shape[0] - N) // N)):
    ...
    for i in range(N, input_shape_i + 1):
        for j in range(N, input_shape_j + 1):
            hash_code = hash_function(input_padded[i : i + N, j : j + N, :])
            if VISUALIZE:
                debug_output[N : N + N, N : N + N] = input_padded[
                    i : i + N, j : j + N, :
                ]
    ...
    for nei in valid_neighbours:
        ...
        if VISUALIZE == True:
            adjacency_vis_list = []
            adjacency_visualization_grid[N * 3 : N * 6, N * 3 : N * 6, :] = cv2.resize(
                get_pattern(nei), (N * 3, N * 3), interpolation=3
            )
        ...
        if VISUALIZE:
            while True:
                for i, direction in enumerate(directions_list):
                    ...
                    adjacency_visualization_grid[
                        direction[0] * N * 3 : direction[0] * N * 3 + N * 3,
                        direction[1] * N * 3 : direction[1] * N * 3 + N * 3,
                        :,
                    ] = cv2.resize(get_pattern(j), (N * 3, N * 3), interpolation=3)
                cv2.rectangle(
                    adjacency_visualization_grid,
                    (N * 3, N * 3),
                    (N * 6 - 1, N * 6 - 1),
    ...
    def collapse_wave_function(output, index):
        ...
        output[index][1] = np.random.choice(
            output[index][2], p=get_probs(output[index][2])
        )
        output[index][0] = LARGE_NUM
        ...
        for i, direction in enumerate(directions_list):
            near_idx = (
                index + (direction[0] - 1) * output_size + (direction[1] - 1)
            ) % len(output)
